...
- [ ] **重连前清理旧连接**：`connect()` 时如果检测到 `socketRef.current` 存在，是否先 `disconnect()` 再创建新连接？
- [ ] **等待连接建立**：是否在 `socket.on('connect')` 回调中发送初始化消息（如 `terminal:create`），而不是立即发送？
- [ ] **竞态条件保护**：快速切换页面时，是否使用 `mountedRef` 等标志位防止竞态？
- [ ] **错误日志上下文**：错误日志是否包含 `sessionId`、`terminalId`、`socketId`、`cause` 等关键信息？

### 服务端层（Hub）

- [ ] **detach 时通知前端**：当调用 `detachSocket()` 时，是否通过 socket 事件通知前端？
- [ ] **rebind 前验证状态**：`rebindSocket()` 时是否检查 terminalId 是否已被其他 socket 占用？
- [ ] **错误消息明确**：错误消息是否包含足够的上下文（如 "Terminal ID is already in use by socket X"）？
...
- [ ] **重连协议明确**：重连时的握手流程是否明确（先 connect → 等待 connect 事件 → 发送 create/rebind）？
- [ ] **状态机一致性**：前端和服务端对连接状态的理解是否一致（idle/connecting/connected/reconnecting/error）？

### 常见陷阱

**陷阱 1：复用已 detach 的 Socket**
- **症状**：`socket_not_connected`、`Terminal ID is already in use`
- **根因**：前端保留了 socketRef，但服务端已经 detach
- **解决**：重连前先 `disconnect()` 并清空 `socketRef`
...
**陷阱 4：快速切换页面的竞态**
- **症状**：偶发的连接错误、状态混乱
- **根因**：组件快速 mount/unmount，导致多个 socket 实例同时存在
- **解决**：使用 `mountedRef` 标志位，在操作前检查组件是否仍然挂载
...
2. **检查服务端日志**：
   - 是否有 `detachSocket` 操作？
   - 是否有 `Terminal ID is already in use` 错误？
   - `rebindSocket` 是否成功？

3. **检查 React DevTools**：
   - `socketRef.current` 是否在预期的时机被清理？
   - 组件 unmount 时 cleanup 是否执行？

4. **检查 Socket.IO DevTools**：
   - Socket 的 `connected` 状态是否符合预期？
   - 事件监听器数量是否异常增长？
